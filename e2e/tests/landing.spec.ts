...
test.describe("landing page", () => {
  test("home renders hero + title", async ({ page }) => {
    ...
    // The landing h1 is split across two lines via <br>; match the level-1
    // heading and assert both halves of the tagline are rendered.
    const hero = page.getByRole("heading", { level: 1 });
    await expect(hero).toBeVisible();
    await expect(hero).toContainText(/AI writes the code/i);
    await expect(hero).toContainText(/Yaver tests it on your real device/i);
