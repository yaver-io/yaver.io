...
test("a data/backend app without a box requires remote setup", () => {
...
test("backend app with a slug still requires a box", () => {
  const r = routeCoding(req({ appKind: "backend", slug: "todos", env: { platform: "ios", backend: NONE } }));
  assert.equal(r.surface, "needs-setup");
});

test("code app, no box, with a backend → remote setup", () => {
...
test("Android with on-device CLI still requires a remote box for new app development", () => {
...
test("a slug without a box still routes to remote setup", () => {
  const r = routeCoding(req({ appKind: "code", slug: "my-app", env: { platform: "ios", backend: GLM } }));
  assert.equal(r.surface, "needs-setup");
  assert.equal(r.screen, "tasks");
});

test("offline box is ignored → requires remote setup", () => {
  const r = routeCoding(
...
test("describeRoute gives a one-liner per surface", () => {
  const r = routeCoding(req({ appKind: "code", env: { platform: "ios", backend: GLM } }));
  assert.match(describeRoute(r), /Yaver box|Managed Cloud/i);
});
