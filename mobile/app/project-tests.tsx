// Project Tests — from the Projects tab, run a project's web test suite
// (yaver-tests/*.test.yaml) on a chosen REMOTE PC via the chromedp runner, watch
// the live log, and read a feature-based highlight report (pass/fail per
// Feature, like match highlights). Self-grow proposes new Features the runner
// can author. Mirrors qa.tsx's transport + report-card pattern.
...
export default function ProjectTestsScreen() {
  ...
  const run = async () => {
    ...
    setReport(null);
    setGrow(null);
    const j = await testkitClient.run(t, { project, dir, env: envFor(), video: true });
    setBusy(false);
      ...
      <ScrollView contentContainerStyle={{ padding: 16, gap: 14 }}>
        ...
        <View style={{ flexDirection: "row", gap: 10 }}>
          <Pressable disabled={busy || running} onPress={run} style={[btn(c.accent), { flex: 1, opacity: busy || running ? 0.6 : 1 }]}>
            <Text style={{ color: "#fff", fontWeight: "700" }}>{running ? "Running…" : "Run Web Tests"}</Text>
          </Pressable>
        ...
        {report && <ReportView report={report} c={c} target={target()} jobId={job?.id} />}
...
function ReportView({ report, c, target, jobId }: { report: TKReport; c: any; target?: TKTarget; jobId?: string }) {
    ...
    <View style={{ gap: 10 }}>
      ...
      {features.map((f, i) => (
        <View key={i} style={{ backgroundColor: c.bgCard, borderRadius: 8, padding: 10, borderLeftWidth: 3, borderLeftColor: f.status === "pass" ? "#2fbf71" : "#ff5d5d" }}>
          ...
          <FeatureMedia feature={f} c={c} target={target} jobId={jobId} />
        </View>
      ))}
    </View>
  );
...
function FeatureMedia({ feature, c, target, jobId }: { feature: TKFeature; c: any; target?: TKTarget; jobId?: string }) {
  ...
  const playClip = async () => {
    ...
    setLoading(true);
    try {
      const a = await testkitClient.artifact(target, jobId, feature.clipPath);
      if (a?.base64) {
