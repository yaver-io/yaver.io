...
- **Convex:** `devices.alias: v.optional(v.string())` (schema.ts#L234). Mutation
...
- **Go CLI:** `yaver alias set/list/clear` → `runAlias` (main.go#L7154); resolver is
...
- **THE ACTUAL GAPS (only two):**
  1. **Auto-naming:** registration sets `name = os.Hostname()` only (main.go:2436;
     Convex registerDevice devices.ts#L341). No smart label. Signals available:
