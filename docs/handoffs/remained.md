...
- Added [MAC_ESCAPE_DEV.md](../archive/2026-04-28/MAC_ESCAPE_DEV.md#L1) as the strategy dump for the Linux / WSL / remote-host to iPhone workflow.
...
- **Yaver Lite on Cloudflare Workers.** Spec is in `docs/mobile/PHONE_EXPORT_PIPELINE.md §Handoff 2.5`. 2-3 days of work — port `phone_backend_http.go` to TS on Workers backed by D1, plus a `POST /phone/projects/deploy-workers` agent helper using the Cloudflare API. Add `{kind: 'cloudflare-workers'}` to `PhonePushTarget`.
