...
- **NEVER run `rm -rf` (or any recursive delete) without verifying the exact target first.** macOS uses a case-insensitive filesystem by default, so `~/workspace` resolves to `~/Workspace` and wipes the entire projects tree — this has already happened once and destroyed a full repo including `.git`. Hard rules before any destructive path op (`rm -rf`, `git clean -fdx`, `find -delete`, `mv` over an existing dir, `trash` of a computed path):
  1. Run `ls -la <path>` first and show the user what is about to be deleted.
  2. Use absolute paths with exact case — never rely on the shell to "figure out" the case.
  3. Stop and ask for confirmation before deleting anything under `$HOME`, `$HOME/Workspace`, a git repo root, any path computed from a variable, or any path transcribed from a user message. "Just cleaning up" is not authorization.
  4. If the intent is to remove one stray subdir, `cd` into the parent, `pwd` to confirm, then delete by simple name — do not pass a long absolute path you built in your head.
  5. Treat an `rm -rf` on anything you did not create in this session as the same risk class as `git push --force` to main.
...
- **Public docs must use machine aliases, not real infra labels.** In README/spec examples prefer `primary`, `selected-machine`, `your-box`, or `example-host`. Do not publish real device nicknames, relay URLs, private domains, or project-specific host labels unless the user explicitly wants that value public.
