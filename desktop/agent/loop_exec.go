...
func phaseCommit(ctx context.Context, l *LoopState, workDir string, aiResp *AIResponse) (string, error) {
	...
	//
	// FixTarget routing:
	//   "" or "main": push directly to ship.branch (legacy)
	//   "branch":     push to a generated autodev/<name>-<ts> branch,
	//                 leaving ship.branch untouched
	//   "pr":         same as "branch" + open a PR via gh CLI
	branch := l.Spec.Ship.Branch
	pushTarget := branch
	mode := strings.ToLower(strings.TrimSpace(l.Spec.Ship.FixTarget))

	if mode == "branch" || mode == "pr" {
		if l.FixBranch == "" {
			l.FixBranch = autoDevBranchName(l.Spec.Name)
		}
		pushTarget = l.FixBranch
	}

	if pushTarget != "" && pushTarget != "local" {
		if err := runGitCtx(ctx, workDir, "push", "origin", "HEAD:"+pushTarget); err != nil {
			...
			fmt.Fprintf(os.Stderr, "[loop %s] git push origin HEAD:%s failed: %v\n",
				l.Spec.Name, pushTarget, err)
		} else if mode == "pr" && pushTarget != branch {
			// First successful push of a fix branch — open the PR. Best-
			// effort: missing gh = degrade to "branch", existing PR =
			// idempotent no-op.
			if _, ghErr := exec.LookPath("gh"); ghErr != nil {
				fmt.Fprintf(os.Stderr,
					"[loop %s] FixTarget=pr but `gh` not on PATH — branch %s pushed without PR\n",
					l.Spec.Name, pushTarget)
			} else {
				openAutoDevPR(ctx, l, workDir, branch, pushTarget, aiResp.Summary)
			}
		}
	}
	return sha, nil
...
// autoDevBranchName generates a safe branch name like
// "autodev/<spec-name>-20260427-001530". Slashes in the spec name are
// preserved (mirroring git ref conventions); other unsafe chars are
// dashified.
func autoDevBranchName(specName string) string {
	ts := time.Now().UTC().Format("20060102-150405")
	...
	return "autodev/" + safe + "-" + ts
}

// openAutoDevPR runs `gh pr create` once per fix branch. Tolerates the
// "PR already exists" case by checking with `gh pr view` first; treats
// any other error as a soft warning so the kick still records success.
func openAutoDevPR(ctx context.Context, l *LoopState, workDir, baseBranch, headBranch, summary string) {
	view := exec.CommandContext(ctx, "gh", "pr", "view", headBranch, "--json", "number")
	view.Dir = workDir
	if err := view.Run(); err == nil {
		return // PR already open for this head — nothing to do
	}
	title := strings.TrimSpace(summary)
	if title == "" {
		title = "autodev: " + l.Spec.Name
	}
	if len(title) > 70 {
		title = title[:70]
	}
	body := fmt.Sprintf("Auto-generated by `yaver loop run %s` (mode: %s).\n\nReview the diff before merging — autodev shipped this without human gate.\n", l.Spec.Name, l.Spec.Mode)
	...
	cmd.Stderr = os.Stderr
	if err := cmd.Run(); err != nil {
		fmt.Fprintf(os.Stderr, "[loop %s] gh pr create %s -> %s failed: %v\n",
			l.Spec.Name, headBranch, baseBranch, err)
	}
}
