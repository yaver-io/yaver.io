...
func (s *HTTPServer) getMCPToolsList() interface{} {
			...
			"inputSchema": map[string]interface{}{
				...
				"properties": map[string]interface{}{
					...
					"runner": map[string]interface{}{
						"type":        "string",
						"description": "Runner ID — claude / codex / opencode / aider / aider-ollama / goose. Empty = agent default.",
					},
	...
	platformTools := []map[string]interface{}{
		...
		{"name": "opencode_config_get", "description": "Read the OpenCode config (~/.config/opencode/opencode.json or env-overridden path). Returns the resolved path, default agent, top-level model + small_model, plus the per-agent build/plan models, all configured providers (with baseURL — useful for remote Tailscale Ollama setups), and a flat list of model identifiers. Optional device_id reads another owned Yaver machine's config.", "inputSchema": map[string]interface{}{"type": "object", "properties": map[string]interface{}{"device_id": map[string]interface{}{"type": "string", "description": "Optional remote device ID"}}}},
		{"name": "opencode_config_set", "description": "Patch the OpenCode config on the local or a remote Yaver machine. Each field is optional; pass empty string to clear. defaultAgent picks the agent invoked when no --agent flag is given. model is the top-level default model id. smallModel is used for cheap helper calls. buildModel and planModel set the model under agent.build / agent.plan in opencode.json. Other config keys (providers, MCP servers, custom agents) stay untouched. Returns the new config summary.", "inputSchema": map[string]interface{}{"type": "object", "properties": map[string]interface{}{"device_id": map[string]interface{}{"type": "string", "description": "Optional remote device ID"}, "default_agent": map[string]interface{}{"type": "string", "description": "Default agent name (e.g. build, plan, or any custom agent)"}, "model": map[string]interface{}{"type": "string", "description": "Top-level default model id"}, "small_model": map[string]interface{}{"type": "string", "description": "Cheap-helper model id"}, "build_model": map[string]interface{}{"type": "string", "description": "Model for agent.build"}, "plan_model": map[string]interface{}{"type": "string", "description": "Model for agent.plan"}}}},
