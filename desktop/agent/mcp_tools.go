...
func (s *HTTPServer) getMCPToolsList() interface{} {
	...
	platformTools := []map[string]interface{}{
		...
		{"name": "runner_auth_browser_start", "description": "Start the interactive plan-OAuth browser/device-auth login flow for Claude Code or Codex on the local or a remote Yaver machine. Uses the user's Claude Pro/Max or ChatGPT Plus/Pro account on their own device; no API key is requested. Returns session plus next_action; by default waits briefly for the runner to print openUrl/code so MCP callers can surface it immediately.", "inputSchema": map[string]interface{}{"type": "object", "required": []string{"runner"}, "properties": map[string]interface{}{"device_id": map[string]interface{}{"type": "string", "description": "Optional remote device ID"}, "runner": map[string]interface{}{"type": "string", "enum": []string{"claude", "claude-code", "codex"}}, "wait_seconds": map[string]interface{}{"type": "integer", "description": "Optional 0-30 seconds to wait for session.openUrl/session.code before returning. Default 5."}}}},
		{"name": "runner_auth_browser_status", "description": "Read the live state of a previously-started runner browser-auth session on the local or a remote machine. Returns next_action so agents know whether to open a URL, submit a pasted code, wait, or stop.", "inputSchema": map[string]interface{}{"type": "object", "required": []string{"session_id"}, "properties": map[string]interface{}{"device_id": map[string]interface{}{"type": "string", "description": "Optional remote device ID"}, "session_id": map[string]interface{}{"type": "string"}}}},
		{"name": "runner_auth_browser_submit_code", "description": "Submit a copied subscription OAuth code/token back into a running runner browser-auth session. Never use this for API keys; Claude Code and Codex runner auth is plan OAuth only.", "inputSchema": map[string]interface{}{"type": "object", "required": []string{"session_id", "code"}, "properties": map[string]interface{}{"device_id": map[string]interface{}{"type": "string", "description": "Optional remote device ID"}, "session_id": map[string]interface{}{"type": "string"}, "code": map[string]interface{}{"type": "string"}}}},
