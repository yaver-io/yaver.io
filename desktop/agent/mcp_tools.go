...
func (s *HTTPServer) getMCPToolsList() interface{} {
	...
	platformTools := []map[string]interface{}{
		...
		// Deploy pipeline
		{"name": "mobile_platform_deploy", "description": "Build/verify/upload Yaver platform-specific mobile surfaces. TV targets are first-class: target=tv runs Android TV + tvOS, android-tv verifies leanback metadata and uploads the shared Play AAB, tvos archives/uploads the standalone Apple TV app. Use dry_run=true to inspect the exact script plan. For build-only verification, pass upload=false.", "inputSchema": map[string]interface{}{"type": "object", "required": []string{"target"}, "properties": map[string]interface{}{"directory": map[string]interface{}{"type": "string", "description": "Repo root. Empty = current yaver repo."}, "target": map[string]interface{}{"type": "string", "enum": []string{"tv", "android-tv", "tvos"}, "description": "Platform surface to deploy."}, "upload": map[string]interface{}{"type": "boolean", "description": "When true, submit store upload after build/verification. Default false for MCP safety."}, "dry_run": map[string]interface{}{"type": "boolean", "description": "Return the plan without running the script."}, "timeout_sec": map[string]interface{}{"type": "integer", "minimum": 60, "maximum": 7200, "description": "Execution timeout. Default 1800 seconds."}}}},
