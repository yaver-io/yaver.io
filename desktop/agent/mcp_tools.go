	...
	droidTools := []map[string]interface{}{
		{
			...
			"inputSchema": map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"device_id": map[string]interface{}{"type": "string", "description": "Owned Yaver machine hosting the Android target (default: this machine)"},
					"device":    map[string]interface{}{"type": "string", "description": "adb serial on that machine (default: first attached device)"},
				},
			},
		},
		{
		...
		{
			"name":        "droid_input",
			"description": "Relay one explicit input event to a user-owned Android device: target (preferred; resolves visible text/content-description/resource-id), tap (x,y), text, key, or swipe. Inspect with droid_ui_elements first. Stop on CAPTCHA, verification, rate-limit, or service block screens; never use this tool to evade them.",
			"inputSchema": map[string]interface{}{
				"type":     "object",
				"required": []string{"type"},
				"properties": map[string]interface{}{
					"type":      map[string]interface{}{"type": "string", "enum": []string{"target", "tap", "text", "key", "swipe"}},
					"target":    map[string]interface{}{"type": "string", "description": "Visible text, content description, or resource id. Ambiguous matches fail without tapping."},
		...
		},
		{
			...
			"description": "Inspect the current Android accessibility hierarchy as structured visible/actionable nodes with labels, resource ids, bounds, and clickable/enabled flags. Read-only. Password nodes are flagged; never persist their values.",
			"inputSchema": map[string]interface{}{
				"type": "object",
				"properties": map[string]interface{}{
					"device":    map[string]interface{}{"type": "string", "description": "adb serial (default: first attached device)"},
					"device_id": map[string]interface{}{"type": "string", "description": "Owned Yaver machine hosting the Android target (default: this machine)"},
					"limit":     map[string]interface{}{"type": "integer", "description": "Maximum nodes, default 120, max 250"},
				},
			},
		},
		{
