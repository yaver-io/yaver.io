...
func (s *HTTPServer) getMCPToolsList() interface{} {
	...
	devTools := []map[string]interface{}{
		...
		// Yaver plan billing (buyer-side — buy/manage Yaver's OWN Workspace/Agent
		// plan from the terminal; distinct from the seller-side lemonsqueezy_* tools)
		{"name": "yaver_billing_status", "description": "Check the user's current Yaver plan: whether they're subscribed, which tier (workspace/byok vs agent/hosted), included active-hours left this month, prepaid wallet balance, and whether managed inference is on. Call this first to know if they've already purchased before offering a checkout.", "inputSchema": map[string]interface{}{"type": "object", "properties": map[string]interface{}{}}},
		{"name": "yaver_billing_checkout", "description": "Get a payment link to subscribe to a Yaver plan. plan=\"workspace\" = Cloud Workspace ($9/mo, bring your own Claude/Codex) — returns a LemonSqueezy checkout URL. plan=\"agent\" = Cloud Agent ($19/mo, included model) — currently points to the dashboard while server-side billing is finalized. The buyer MUST pay with the same email they signed into Yaver with. Call yaver_billing_status first to avoid offering a plan they already have.", "inputSchema": map[string]interface{}{"type": "object", "properties": map[string]interface{}{"plan": map[string]interface{}{"type": "string", "description": "\"workspace\" ($9 BYO) or \"agent\" ($19 included model). Default: workspace."}}}},
