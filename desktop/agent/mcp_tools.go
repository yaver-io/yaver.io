...
func (s *HTTPServer) getMCPToolsList() interface{} {
	...
	platformTools := []map[string]interface{}{
		...
		{"name": "yaver_auth_list_identities", "description": "List every OAuth provider (Apple / Google / Microsoft / email) linked to the currently signed-in Yaver account. Returns each provider's email and which one is primary. Safe to call any time.", "inputSchema": map[string]interface{}{"type": "object", "properties": map[string]interface{}{}}},
		{"name": "yaver_auth_link_start", "description": "Connect an ADDITIONAL OAuth provider to the currently signed-in account — e.g., user signed up with Apple but wants to also sign in with Google. Returns {url, qr_ascii, link_token, expires_at_ms}. Render the URL + QR; the user opens it, signs in with that provider, and Yaver binds the provider to the existing account. Call yaver_auth_link_wait afterwards to confirm.", "inputSchema": map[string]interface{}{"type": "object", "required": []string{"provider"}, "properties": map[string]interface{}{"provider": map[string]interface{}{"type": "string", "description": "Provider to link: apple | google | microsoft"}}}},
	...
	autodevTools := []map[string]interface{}{
		{
			...
			"inputSchema": map[string]interface{}{
				...
				"properties": map[string]interface{}{
					...
					"engine":           map[string]interface{}{"type": "string", "enum": []string{"claude", "codex", "hybrid"}, "description": "High-level engine. 'claude' (default) uses Claude Code end-to-end. 'hybrid' uses Claude as a planner and a local Aider+Ollama implementer to cut API spend ~80-95%."},
					...
					"planner":          map[string]interface{}{"type": "string", "description": "Hybrid layering: planner agent[:model], e.g. 'claude:opus' or 'codex'. Set this OR implementer to force engine=hybrid; leaving both empty preserves the default no-slicing behaviour."},
					"implementer":      map[string]interface{}{"type": "string", "description": "Hybrid layering: implementer agent[:model], e.g. 'claude:sonnet', 'codex', 'aider-ollama', 'aider-ollama:qwen2.5-coder:14b'."},
