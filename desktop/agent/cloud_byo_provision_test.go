...
func TestHetznerCreateServerCustom_ImageAndRepoClone(t *testing.T) {
	...
	if !strings.Contains(gotUserData, "git -C /home/yaver/Workspace/repos clone --depth 1 https://github.com/acme/app.git") {
		t.Fatalf("user_data missing the yaver-user ~/Workspace shallow clone; got:\n%s", gotUserData)
	}
	if !strings.Contains(gotUserData, "/home/yaver/Workspace/worktrees") {
		t.Fatalf("user_data missing the managed worktrees directory; got:\n%s", gotUserData)
	}
