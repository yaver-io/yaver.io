...
run_ci() {
  ...
  if need npm; then
    ...
    [[ -d mobile   ]] && step "mobile typecheck"   bash -c "cd mobile && (test -d node_modules || npm ci) && cd .. && node --test scripts/generate-sdk-manifest.test.mjs && node scripts/generate-sdk-manifest.mjs --check && cd mobile && npx tsc --noEmit"
