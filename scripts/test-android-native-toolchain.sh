...
grep -q 'GRADLE_OPTS=.*-Xmx768m' "$DEPLOY"
...
grep -q -- "-Dorg.gradle.jvmargs=-Xmx1024m" "$DEPLOY"
...
grep -q -- '-Xmx1024m -XX:MaxMetaspaceSize=256m -XX:+UseSerialGC' "$GRADLE_MEMORY_HELPER"
